...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  return (
    <div id="admin-dashboard-root" className="min-h-screen bg-slate-950 text-slate-100 flex flex-col lg:flex-row">
      {/* Admin Sidebar */}
      <AdminSidebar
        activeTab={activeTab}
        onSelectTab={(tab) => {
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-900/60 overflow-y-auto">
        {/* Top Navbar */}
        <header className="h-16 px-4 sm:px-8 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              onClick={onExitAdmin}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl transition-colors cursor-pointer"
            >
